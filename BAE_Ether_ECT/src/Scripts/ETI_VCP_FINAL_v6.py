...
class ChannelConfig:
    ...
    load: int # think this for the absolute mode (BNC) 
...
class ETi300:
    ...
    def __init__(self, com_port: str = 'COM4', baud_rate: int = None, timeout: float = None):
        ...
        # ChannelConif(Connector type, frequency, phase, gain_x, gain_y, hp_filter, lp_filter, load)
        self.channel_configs = {
            0: ChannelConfig(ConnectorType.LEMO_1_BRIDGE,2000000, 0, 400, 400, 0, 0,0),
            1: ChannelConfig(ConnectorType.OFF,26000, 0, 600, 600, 0, 0,0),
            2: ChannelConfig(ConnectorType.OFF,40000, 0, 600, 300, 0, 0,0),
            3: ChannelConfig(ConnectorType.OFF,25000, 0, 600, 100, 0, 0,0)
        }
    ...
    def _send_xml_acquire(self, xml_command: str):
        ...
        # Flush any stale data before sending (matching MATLAB)
        self.serial_port.reset_input_buffer()
        time.sleep(0.5)
    ...
    def perform_bootloader_handshake(self):
        """Perform bootloader handshake and restart ETi (matching MATLAB wakeupETI logic)"""
        ...
        # Try the write
        try:
            self.serial_port.write(xml_packet)
            print("Wake command sent.")
            time.sleep(0.5)
            
            # CRITICAL: Check if device is still alive
            try:
                self.serial_port.flush()  # This will fail if device disconnected
                self.serial_port.reset_input_buffer()
                
                # Extra verification - try to check if port is actually open
                if not self.serial_port.is_open:
                    device_rebooted = True
                else:
                    print("Device remained connected, no reconnect needed.")
                    device_rebooted = False
            except:
                print("Device disconnected after wake command (expected).")
                device_rebooted = True
        ...
        # If device rebooted, we MUST reconnect
        if device_rebooted:
            print("Waiting for device to reboot...")
            time.sleep(1.5)  # Give it time to fully reboot
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    self.serial_port.reset_input_buffer()
                    connected = True
                    print(f"✓ ETI reconnected successfully to {self.com_port}")
                    break
                except Exception as e:
    ...
    def configure_channel(self, channel: int, config: Optional[ChannelConfig] = None):
        ...
        commands = [
            ...
            f'<CHANNEL>{channel}\n<GAIN_X>{config.gain_x}</GAIN_X>\n</CHANNEL>',
            f'<CHANNEL>{channel}\n<GAIN_Y>{config.gain_y}</GAIN_Y>\n</CHANNEL>',
            f'<CHANNEL>{channel}\n<HP>{config.hp_filter}</HP>\n</CHANNEL>',
            f'<CHANNEL>{channel}\n<LP>{config.lp_filter}</LP>\n</CHANNEL>',
            f'<CHANNEL>{channel}</CHANNEL>',
            f'<CONNECTOR>{config.connector.value}</CONNECTOR>',
            f'<LOAD>{config.load}</LOAD>' 
