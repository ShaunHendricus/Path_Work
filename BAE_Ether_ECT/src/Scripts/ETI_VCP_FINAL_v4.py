# -*- coding: utf-8 -*-
"""
Created on Thu Jun 26 10:30:27 2025
...
"""
Changes to be made:
    1. different bits of the packets need to be read.
    2. need to be able to stream data from the 8 channel array.
...
class ChannelConfig:
    ...
    hp_filter: int  # in 100ths of a Hz from 0.01Hz steps to 2kHz (1 to 200000).
...
class ETi300:
    ...
    def __init__(self, com_port: str = 'COM4', baud_rate: int = None, timeout: float = None):
        ...
        self.channel_configs = {
            0: ChannelConfig(ConnectorType.LEMO_1_BRIDGE,2000000, 0, 400, 400, 0, 0),
            1: ChannelConfig(ConnectorType.LEMO_1_BRIDGE,26000, 0, 600, 900, 0, 0),
            2: ChannelConfig(ConnectorType.LEMO_1_BRIDGE,40000, 0, 600, 900, 0, 0),
            3: ChannelConfig(ConnectorType.LEMO_1_BRIDGE,25000, 0, 600, 900, 0, 0)
        }
    ...
    def configure_channel(self, channel: int, config: Optional[ChannelConfig] = None):
        ...
        # Send each parameter
        commands = [
            f'<CHANNEL>{channel}\r<CONNECTOR>{config.connector.value}</CONNECTOR>',
    ...
    def _parse_format5_packet(self, data: bytes, start_idx: int) -> Dict[str, int]:
        ...
        # Validate status bytes
        if data[i] != 0x80 or data[i + 1] != 0x7F:
            print(f"  DEBUG: Invalid status bytes at {i}: {hex(data[i])}, {hex(data[i+1])}")
            return None
       
        # Print the full 53-byte packet
        packet_hex = ' '.join(f'{data[i+j]:02x}' for j in range(53))
        print(f"  DEBUG: Full packet: {packet_hex}")
        # Parse X values
        rawX1 = data[i + 2] + (data[i + 3] * 256) + (data[i + 4] * 256 * 256) + (data[i + 5] * 256 * 256 *256)
        X1 = rawX1 - (2**32 if rawX1 >= 2**31 else 0) # Two's complement for 32-bit to handle signed integers
        
        rawX2 = data[i + 10] + (data[i + 11] * 256) + (data[i + 12] * 256 * 256) + (data[i + 13] * 256 * 256 *256)
        X2 = rawX2 - (2**32 if rawX2 >= 2**31 else 0) # Two's complement for 32-bit to handle signed integers
        
        rawX3 = data[i + 18] + (data[i + 19] * 256) + (data[i + 20] * 256 * 256) + (data[i + 21] * 256 * 256 *256)
        X3 = rawX3 - (2**32 if rawX3 >= 2**31 else 0) # Two's complement for 32-bit to handle signed integers
        
        rawX4 = data[i + 26] + (data[i + 27] * 256) + (data[i + 28] * 256 * 256) + (data[i + 29] * 256 * 256 *256)
        X4 = rawX4 - (2**32 if rawX4 >= 2**31 else 0) # Two's complement for 32-bit to handle signed integers
        
        # Parse Y values
        rawY1 = data[i + 6] + (data[i + 7] * 256) + (data[i + 8] * 256 * 256) + (data[i + 9] * 256 * 256 *256)
        Y1 = rawY1 - (2**32 if rawY1 >= 2**31 else 0) # Two's complement for 32-bit to handle signed integers
        
        rawY2 = data[i + 14] + (data[i + 15] * 256) + (data[i + 16] * 256 * 256) + (data[i + 17] * 256 * 256 *256)
        Y2 = rawY2 - (2**32 if rawY2 >= 2**31 else 0) # Two's complement for 32-bit to handle signed integers
        
        rawY3 = data[i + 22] + (data[i + 23] * 256) + (data[i + 24] * 256 * 256) + (data[i + 25] * 256 * 256 *256)
        Y3 = rawY3 - (2**32 if rawY3 >= 2**31 else 0) # Two's complement for 32-bit to handle signed integers
        
        rawY4 = data[i + 30] + (data[i + 31] * 256) + (data[i + 32] * 256 * 256) + (data[i + 33] * 256 * 256 *256)
        Y4 = rawY4 - (2**32 if rawY4 >= 2**31 else 0) # Two's complement for 32-bit to handle signed integers
    ...
    def _update_plot(self, frame):
        ...
        # Read available data
        if self.serial_port.in_waiting < 53:  # format 5
            return list(self.lines.values())
        ...
        pkt_starts = [start for start in pkt_starts if start + 52 < len(data)]
