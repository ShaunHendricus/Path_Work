...
Changes made:
    ...
    2. Fixed _parse_format5_packet: replaced incorrect 3-byte manual extraction with
       struct.unpack_from('<i', ...) 4-byte int32 reads, matching MATLAB's
       typecast(uint8(packet(3:6)), 'int32') etc. Offsets now align correctly:
       X1@+2, Y1@+6, X2@+10, Y2@+14, X3@+18, Y3@+22, X4@+26, Y4@+30.
"""
import serial
import time
import numpy as np
...
class ETi300:
    ...
    def start_streaming(self, duration: float = 500, output_format: int = 5):
        """
        Start streaming data from ETi 300 using PyQtGraph for real-time plotting.

        Architecture:
          - Serial reading runs in the main thread tight loop
          - PyQtGraph plot updates driven by a QTimer at fixed FPS
          - QApplication.processEvents() keeps the GUI responsive without blocking serial
          - All data accumulated in numpy arrays (no data dropped)

        Args:
            duration: Streaming duration in seconds
            output_format: ETi output format (default: 5 for X,Y pairs)
        """
        PLOT_FPS = 30  # target frames per second for plot refresh
        ...
        # Growing lists — all data kept, same as MATLAB Xarray/Yarray
        t_data  = []
        x1_data = []; y1_data = []
        x2_data = []; y2_data = []
        x3_data = []; y3_data = []
        ...
        # QTimer drives plot refresh independently of serial read rate
        last_plot_time = [self.start_time]  # list so closure can mutate it
        plot_interval = 1.0 / PLOT_FPS

        def _refresh_plot():
            now = time.time()
            if now - last_plot_time[0] < plot_interval:
                return
            if not t_data:
                return
            t_arr  = np.array(t_data,  dtype=np.float64)
            x1_arr = np.array(x1_data, dtype=np.float64)
            y1_arr = np.array(y1_data, dtype=np.float64)
            x2_arr = np.array(x2_data, dtype=np.float64)
            y2_arr = np.array(y2_data, dtype=np.float64)

            # setData is the PyQtGraph equivalent of set_xdata/set_ydata — very fast
            self.lines['x1'].setData(t_arr, x1_arr)
            self.lines['y1'].setData(t_arr, y1_arr)
            self.lines['x2'].setData(t_arr, x2_arr)
            self.lines['y2'].setData(t_arr, y2_arr)

            # Keep x-axis tracking elapsed time
            elapsed = now - self.start_time
            for ax in self.axes.values():
                ax.setXRange(0, elapsed, padding=0)
                ...
                if self.serial_port.in_waiting >= 53:
                    ...
                    while i < len(raw) - 1:
                        if raw[i] == 0x80 and raw[i + 1] == 0x7F:
                            if i + 52 >= len(raw):
                                break
                            values = self._parse_format5_packet(raw, i)
                            elapsed = now - self.start_time
                            t_data.append(elapsed)
                            x1_data.append(values['x1']); y1_data.append(values['y1'])
                            x2_data.append(values['x2']); y2_data.append(values['y2'])
                            x3_data.append(values['x3']); y3_data.append(values['y3'])
                            x4_data.append(values['x4']); y4_data.append(values['y4'])
                            i += 53
                        else:
                            i += 1

                # Update plot and service GUI events — non-blocking, ~1ms
                _refresh_plot()
        ...
        if t_data:
            self.lines['x1'].setData(np.array(t_data), np.array(x1_data))
            self.lines['y1'].setData(np.array(t_data), np.array(y1_data))
            self.lines['x2'].setData(np.array(t_data), np.array(x2_data))
            self.lines['y2'].setData(np.array(t_data), np.array(y2_data))
            for ax in self.axes.values():
                ax.enableAutoRange()

        print(f'Done streaming. {len(t_data)} samples collected.')

        # Store in data_queues so save_data() still works
        for lst, key in [
            (t_data, 't'), (x1_data, 'x1'), (y1_data, 'y1'),
            (x2_data, 'x2'), (y2_data, 'y2'), (x3_data, 'x3'),
            (y3_data, 'y3'), (x4_data, 'x4'), (y4_data, 'y4')
        ]:
            self.data_queues[key].clear()
            self.data_queues[key].extend(lst)

        # Keep window open after streaming ends so data can be inspected
