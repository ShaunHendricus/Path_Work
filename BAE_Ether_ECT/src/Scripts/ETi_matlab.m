...
% hdr = send_and_check_xml(eti, '<USB_OUTPUT>4</USB_OUTPUT>', 1);
% pause(1)
% if isempty(hdr)
%     error('No bootloader detected – check power & port');
% else
% %     hexStr = sprintf('%02X ', hdr);  % two-digit hex with spaces
% %         fprintf('HEX  : %s\n', hexStr);
%     fprintf('Bootloader present, version info : %s\n', hdr);
% 
% end

% Tell the loader to run the main firmware…
send_and_check_xml(eti, '<USB_OUTPUT>99</USB_OUTPUT>', 1);
pause(1)
...
hdr2 = send_and_check_xml(eti, '<USB_OUTPUT>0</USB_OUTPUT>', 1); %stop any streaming of data
pause(1)

hdr2 = send_and_check_xml(eti, '<USB_RATE>1000</USB_RATE>', 1); %sample rate for ETi is 16,000 Hz, so sending 1000 will give 16000/1000 (16 Hz)
pause(1)
...
%      if eti.NumBytesAvailable < 50 %format 8
        pause(0.005);
        continue;
    end
    ...
    % find all indices where bytes == [0x80, 0x7F] %format 8
    % find all indices where bytes == [0x50, 0xAF] %format 5
%      pkt_starts = find(data(1:end-1)==128 & data(2:end)==127);%format 8
    pkt_starts = find(data(1:end-1)==128 & data(2:end)==127);%format 5
   
    pkt_starts = pkt_starts(pkt_starts + 49 <= numel(data)); %fomat 8
%      pkt_starts = pkt_starts(pkt_starts + 49 <= numel(data)); %fomat 5
    
    for k = 1:numel(pkt_starts)
        i = pkt_starts(k);

        pkt = data(i : i+49);          % 1×26 uint8
...
%% ------------------------------------------------------------------------
function header = send_and_check_xml(ser, xml_command, timeout)
    % Sends an XML command (<USB_OUTPUT>n</…>) and waits for <INSTRUMENT>…
    % Alexis function
    % use this function only to read the instrumentation config
    if nargin<3, timeout = 1; end
    header = '';
    ...
    %cmd = uint8([1, 0, 0x08]);  % Predefined command (0x01, 0x00, code)
    %write(ser, cmd, "uint8")
...
%     cmd = uint8([1, 0x0A]);  % Predefined command (0x01, 0x00, code)
%     write(ser, cmd, "uint8")
