...
% NOTE: Commenting this out - we'll start streaming AFTER parameter verification
...
%load, connector,timeout
set_channel_basic(eti,0,1000000,0,600,600,0,'Lemo 1-Bridge',1)
% pause(0.1);
...
% VERIFY PARAMETERS WERE SET CORRECTLY (before starting streaming)
verify_channel_config(eti, 0);

% NOW START STREAMING FOR DATA ACQUISITION
send_and_check_xml(eti, '<USB_OUTPUT>5</USB_OUTPUT>', 1);
pause(0.5);
...
[X, Y, Z,data] = StreamETiDataLivePlot(eti, 60);
...
% function response = set_channel_basic(serialObj, channel, frequency, phase, gainX, gainY, HP, LP, load, connector, timeout)
 function response = set_channel_basic(serialObj, channel, frequency, phase, gainX, gainY, load, connector, timeout)
    ...
    %       </CHANNEL>
    disp('configuring ETI be patient')
...
%% NEW FUNCTIONS - Parameter Verification (from Python script logic)
